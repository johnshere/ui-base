...
import { buildConfig, type BuildModule } from '../config';
...
export async function generateTypesDefinitions() {
  ...
  const tasks = sourceFiles.map(async (sourceFile) => {
    const relativePath = path.relative(packagesPath, sourceFile.getFilePath());
    consola.trace(
      chalk.yellow(
...
function customModuleResolution(
  ...
  return {
    resolveModuleNames(moduleNames, containingFile) {
      ...
      try {
        containingFileRealPath = fs.realpathSync(containingFile, 'utf-8');
      } catch {
        // 找不到没关系，因为找不到的文件是在 Project 中存储的虚拟文件
      }
      moduleNames = moduleNames.map(removeTsExtension).map(mapModuleAlias);
      // console.log('=========================');
      // console.log(
...
function mapModuleAlias(moduleName: string) {
  ...
  // 正确索引到 @src
  moduleName = moduleName.replace('@src', packagesPath);
  return moduleName;
}
...
export const outputContentReplacer = (option: {
  ...
  return (
    content
       ...
       */
      .replace(/(['"]@?)(vue[23])(\/.*)?(['"])/g, (match, p1, p2, p3, p4) => {
        return `${p1}vue${p3 || ''}${p4}`;
      })
      // 替换 @src 别名
      .replace(/@src\//g, getAliasRealPath(packagesPath) + '/')
  );
};
...
export async function copyTypes() {
  async function copyTypes(module: BuildModule) {
    const out = buildConfig[module].path;
    await new Promise((resolve) => {
      src([
        path.join(GENERATE_TYPES_DIR, '**/*.d.ts'),
        path.join(packagesPath, '**/*.d.ts'),
      ])
        .pipe(dest(out))
        .on('end', resolve);
    });
    if (!IS_VUE2) {
      // 在Vue3环境下将element-ui替换为element-plus
      const globalPath = path.join(out, 'global.d.ts');
      let globalTypes = fs.readFileSync(globalPath, 'utf-8');
      globalTypes = (globalTypes as string).replace(
        /\('element-ui'\)\['([^']+)'\]/g,
        (_, comName) => `('element-plus')['El${comName}']`
      );
      fs.writeFileSync(globalPath, globalTypes);
    }
  }
